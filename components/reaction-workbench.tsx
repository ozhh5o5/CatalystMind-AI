...
type CandidateRow = {
  ...
  predictedToxicity?: number | null
  predictedSynthesizability?: number | null
  uncertaintyActivity?: number | null
  uncertaintySelectivity?: number | null
  uncertaintyStability?: number | null
  uncertaintyToxicity?: number | null
  uncertaintySynthesizability?: number | null
  confidence: number
  reasoning: string
...
export function ReactionWorkbench({ reaction: initial }: { reaction: ReactionPayload }) {
  ...
  const recommendedCandidates = useMemo(() => {
    // Bayesian Active Learning: high predicted activity AND high uncertainty
    return reaction.candidates
      .filter((c) => (c.uncertaintyActivity ?? 0) > 0.2 && c.predictedActivity > 0.6)
      .sort((a, b) => (b.uncertaintyActivity ?? 0) - (a.uncertaintyActivity ?? 0))
      .slice(0, 3)
  }, [reaction.candidates])
            ...
            <Card className="border-violet-200 bg-violet-50/50 dark:border-violet-900/50 dark:bg-violet-900/10">
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-violet-700 dark:text-violet-300">
                  ...
                  Bayesian Active Learning Recommendations
                </CardTitle>
                <CardDescription className="text-violet-600/80 dark:text-violet-400/80">
                  The AI identifies these candidates as having high potential payoff (high activity) but high uncertainty. Testing these next will mathematically maximize informational yield.
                </CardDescription>
              </CardHeader>
                ...
                {recommendedCandidates.map(c => (
                  <div key={c.id} className="rounded-lg border bg-background p-4 shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      <Link href={`/candidates/${c.id}`} className="font-semibold text-primary hover:underline line-clamp-1">{c.name}</Link>
                      <Badge variant="outline" className="bg-violet-100 text-violet-700 dark:bg-violet-900/50 dark:text-violet-300 border-violet-200">Next</Badge>
                    </div>
                    <div className="text-sm text-muted-foreground flex justify-between">
                      <span>Activity:</span>
                      <span className="font-medium text-foreground">{c.predictedActivity.toFixed(2)} <span className="text-violet-500">±{(c.uncertaintyActivity ?? 0).toFixed(2)}</span></span>
                    </div>
                    <Button size="sm" variant="secondary" className="w-full mt-3" onClick={() => {
                      setLogCandidateId(c.id)
                      setLogOpen(true)
                    }}>
                      Log Experiment
                    </Button>
