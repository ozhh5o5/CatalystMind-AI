...
function mockPredictProperties(
  ...
  uncertaintyActivity: number
  uncertaintySelectivity: number
  uncertaintyStability: number
  uncertaintyToxicity: number
  uncertaintySynthesizability: number
  confidence: number
  ...
  const tox = 0.1 + 0.5 * unitFloat(seed, 7)
  const synth = 0.4 + 0.6 * unitFloat(seed, 8)
  
  // Bayesian uncertainty mapping - high uncertainty for generative, lower for knowns
  const isGenerative = /Modified/.test(candidate.name || "");
  const uncBase = isGenerative ? 0.3 : 0.05;
  const uncAct = uncBase + 0.2 * unitFloat(seed, 9);
  const uncSel = uncBase + 0.15 * unitFloat(seed, 10);
  const uncStab = uncBase + 0.25 * unitFloat(seed, 11);
  const uncTox = uncBase + 0.1 * unitFloat(seed, 12);
  const uncSynth = uncBase + 0.3 * unitFloat(seed, 13);
  ...
  const rationale: Record<string, string> = {
    ...
    toxicity: "In-silico Ames test and aquatic toxicity proxy via structural alerts.",
    synthesizability: "Retrosynthetic graph depth to Sigma-Aldrich precursor database.",
  }
  return { 
    activity: a, selectivity: s, stability: st, yield: y, 
    toxicity: tox, synthesizability: synth,
    uncertaintyActivity: uncAct, uncertaintySelectivity: uncSel, 
    uncertaintyStability: uncStab, uncertaintyToxicity: uncTox, 
    uncertaintySynthesizability: uncSynth,
