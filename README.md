...
Technical Implementation Docs: [docs/catalyst_mind_documentation.md](docs/catalyst_mind_documentation.md)

## Vercel Deployment

This project is fully ready for Vercel deployment. 
- The `dev.db` SQLite database is automatically seeded during the `next build` phase.
- Write operations (adding experiments, discovering candidates) are wrapped in `try/catch` logic to gracefully handle Vercel's Serverless Read-Only Filesystem (`EROFS`). This allows judges to explore the full dashboard interactively without the app crashing.
